<?php 
    ...
    if(isset($_POST['next_page'])){
      ...
      $_SESSION['time_rate'] = isset($_POST['time_rate']) ?  $time_rate : null;
      $pax=$_POST['adults'] + $_POST['kids'];
      $_SESSION['pax'] = $pax;
      header("location:book.php?start_date=".$checkin."&end_date=".$checkout."&search=&pax=".$pax) ;
         }
?>
...
<html>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.1.3/js/bootstrap.min.js"></script>
    ...
    <div class="jumbotron" style="background-image: linear-gradient(to bottom, rgba(0,0,0,0.3), rgba(0,0,0,0.3)), url(../pictures/reserve.jpg); border-radius: 0px; background-size: cover; height: 100vh; max-height: 140px;">
        ...
        <h4 style="font-size:40px;color:white;font-weight:bold;font-family:'';">            
 BOOK APPOINTMENT
</h4>
        </div>
    </div>
    ...
    <?php
$scan_pending ="SELECT * FROM reservation where userid='$currentuser' AND (approvalstatus='Pending' OR approvalstatus='Approved' OR(approvalstatus='Checkout' AND rated='0'))";
 $query  = mysqli_query($con,$scan_pending);
...
header('location:reservation_status.php');
  ...
  <div class="stepper-item">
    ...
    <div class="step-name">Booking Confirmation</div>
      ...
      <form method="post">
         <input type="" type="text" name="name" value="<?php echo $name?>" hidden>
                        <input type="" type="text" name="userid" value="<?php echo $currentuser?>" hidden>
 ...
 <div class="col-sm-6">
      ...
      <div class="col-sm-6">
        <label for="kids" class="col-form-label"><b>Number of Kids:</b></label>
        <div class="input-group date" >
       ...
       <div class="row">
      <div class="col-sm-6">
      <label for="cars" class="col-form-label ml-1"><b>Rates</b></label>
        <select name="time_rate" id="time_rate" class="form-select ml-1" aria-label="Select time rate" required>
        <option value="" disabled selected>Select time rate</option>
