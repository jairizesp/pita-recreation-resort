...
if(isset($_POST['code'])){
    $email=$_POST['email'];
    $code=$_POST['code'];
    $select="SELECT * FROM user where email='$email' AND status='Pending'";
    $result2 = $con->query($select);
     if ($result2->num_rows > 0) {
        while ($row = $result2->fetch_assoc()) { 
            $user_code =$row['code'];
             if($code==$user_code){
            $sql_update="UPDATE user set status='Verified' where email='$email'";
                ...
                <div class="card-header p-4">
                    <div class="card-body">
                        ...
                        <div class="container">
                           ...
                           <div class="col-md-12">
                            <input type="text" name="email" placeholder="Enter code" class="form-control col-md-3 text-center"value='<?php echo $_GET['id']?>' hidden>
